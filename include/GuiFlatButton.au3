...
Func GuiFlatButton_Create($sText, $x, $y, $w = -1, $h = -1, $style = 0)
	...
	GUICtrlSetStyle($buttonID, BitOR($WS_TABSTOP, $BS_NOTIFY, $BS_OWNERDRAW), $WS_EX_COMPOSITED) ; Set the ownerdrawn flag
...
Func GuiFlatButton_Redraw($controlID)
	If Not $aGuiFlatButton[0][0] Then Return 0
	$controlID = GuiFlatButton_prevControlId($controlID)
...
Func GuiFlatButton_ChildHandler($hwnd, $iMsg, $wParam, $lParam, $idx, $pData)
	...
	Switch $iMsg
		Case $WM_DRAWITEM
;~ 			$counter += 1
;~ 			ConsoleWrite("draw item" & $counter & @CRLF)
